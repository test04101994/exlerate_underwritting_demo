...
function StreamingText({
  ...
  return (
    <>
      <FormattedMessageContent content={displayed} />
      {!done && (
    ...
    <div className="flex-1 flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 max-h-full">
        {messages?.map((message) => {
          ...
          // Only the single designated streaming message animates; everything else is instant
          const shouldStream = message.id === streamingIdRef.current;
      ...
      <div className="border-t border-border p-4 bg-card sticky bottom-0">
        <div className="flex items-center space-x-2">
          <Input
            type="text"
            placeholder="Type your message or give instructions to agents..."
            value={inputValue}
