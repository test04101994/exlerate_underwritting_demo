...
import { getSlipDataById } from "@shared/slip-data";
...
export function MultiDocumentViewer({ sessionId, className, onWidthChange, initialWidth = 320, submissionId = "SLP-2025-001", caseId, workflowType, highlight, propertyLocation }: DocumentViewerProps) {
  ...
  const clientName = useMemo(() => {
    const slipData = getSlipDataById(submissionId || '');
    return slipData?.lead_underwriter || slipData?.token || 'Case';
  }, [submissionId]);
  ...
  const { data: caseDocuments, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const endpoint = `/api/documents/${caseId || submissionId}`;
      ...
      content: (
        <div className="flex flex-col h-full gap-2">
          <div className="flex items-center justify-between flex-shrink-0">
            <div>
              <h3 className="font-medium">{doc.name}{!isJiraWorkflow ? ` - ${clientName}` : ''}</h3>
            ...
            {(doc.type === 'doc' || doc.type === 'docx') ? (
              ...
              <PDFHighlightViewer
                url={isJiraWorkflow
                  ...
                  : `/api/documents/${caseId || submissionId}/${doc.name}`
                }
  ...
  }, [caseDocuments, clientName, caseId, submissionId, zoomLevel, highlight, isJiraWorkflow]);
