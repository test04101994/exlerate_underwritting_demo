...
interface UnifiedDataExtractionFormProps {
  sessionId: string;
  workflowType: 'submission' | 'slip' | 'jira';
  caseId?: string;
  ticketKey?: string;
      ...
      return [
        {
          ...
          fields: [
            { key: 'first_name', label: 'First Name', type: 'text', confidence: 'High', section: 'client' },
            { key: 'middle_name', label: 'Middle Name', type: 'text', confidence: 'High', section: 'client' },
            { key: 'surname', label: 'Surname', type: 'text', confidence: 'High', section: 'client' },
            { key: 'date_of_birth', label: 'Date of Birth', type: 'text', confidence: 'High', section: 'client' },
            { key: 'occupation', label: 'Occupation', type: 'text', confidence: 'High', section: 'client' }
          ]
        },
        {
          title: "Broker Information",
          fields: [
            { key: 'intermediary_name', label: 'Intermediary', type: 'text', confidence: 'High', section: 'broker' },
            { key: 'broker_contact_name', label: 'Broker Contact', type: 'text', confidence: 'High', section: 'broker' },
            { key: 'broker_email', label: 'Broker Email', type: 'email', confidence: 'High', section: 'broker' },
            { key: 'broker_ref_hu_number', label: 'Broker Reference', type: 'text', confidence: 'High', section: 'broker' }
          ]
        },
        {
          title: "Property Details",
          fields: [
            { key: 'property_type', label: 'Property Type', type: 'text', confidence: 'High', section: 'property' },
            { key: 'property_address_line1', label: 'Property Address', type: 'text', confidence: 'High', section: 'property' },
            { key: 'property_city', label: 'City', type: 'text', confidence: 'High', section: 'property' },
            { key: 'property_postcode', label: 'Postcode', type: 'text', confidence: 'High', section: 'property' },
            { key: 'year_built', label: 'Year Built', type: 'text', confidence: 'Medium', section: 'property' },
            { key: 'listed_building', label: 'Listed Building', type: 'text', confidence: 'Medium', section: 'property' }
          ]
        },
        {
          title: "Coverage & Financial",
          fields: [
            { key: 'cover_type', label: 'Coverage Type', type: 'text', confidence: 'High', section: 'coverage' },
            { key: 'target_premium', label: 'Target Premium', type: 'text', confidence: 'High', section: 'coverage' },
            { key: 'building_sums_insured', label: 'Building Sum Insured', type: 'text', confidence: 'High', section: 'coverage' },
            { key: 'content_sums_insured', label: 'Content Sum Insured', type: 'text', confidence: 'High', section: 'coverage' },
            { key: 'total_jewellery_value', label: 'Jewellery Value', type: 'text', confidence: 'Medium', section: 'coverage' }
          ]
        }
      ];
...
export function UnifiedDataExtractionForm({ 
  ...
  const { data: sourceData, isLoading, error } = useQuery({
    queryKey: [`/api/${workflowType === 'jira' ? 'slip-data' : workflowType === 'slip' ? 'slip-data' : 'submission-data'}`, dataSourceId],
    queryFn: async () => {
      let endpoint = '';
      
      if (workflowType === 'jira') {
        // Jira workflows use slip data structure
        endpoint = `/api/slip-data/${dataSourceId}`;
      } else if (workflowType === 'slip') {
        endpoint = `/api/slip-data/${dataSourceId}`;
      } else {
        endpoint = `/api/submission-data/${dataSourceId}`;
      }
  ...
  const getWorkflowTitle = () => {
    switch (workflowType) {
      case 'submission': return 'Submission Data Extraction Review';
