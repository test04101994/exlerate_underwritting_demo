                ...
                {(agent.type === 'extraction' || agent.type === 'data_extraction' || agent.name.includes('Data Extraction')) && (
                  <div className="mt-2">
                    <Button
                      ...
                      disabled={session?.workflowType !== 'submission' && session?.workflowType !== 'slip' && agent.status !== 'completed' && session?.status !== 'pending_data_extraction' && session?.status !== 'completed'}
                      onClick={() => {
                        if (session?.sessionId?.startsWith('JIR-') || session?.workflowType === 'submission' || session?.workflowType === 'slip') {
