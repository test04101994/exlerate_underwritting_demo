...
import { Send, Pause, Square, Edit2, Rocket, ChevronRight, ChevronLeft, TrendingUp, MessageSquare, Clock, FileText } from "lucide-react";
...
export default function WorkflowPage() {
  ...
  const isSlipAtPolicyExtractionGate = wfType === 'slip' && !dataExtractionCompleted &&
    workflowStatus === 'pending_data_extraction';
  const shouldShowDataExtractionForm = showDataExtractionForm || isSlipAtPolicyExtractionGate;
  ...
  const timelineCaseId = (workflowData as any)?.session?.caseId || (workflowData as any)?.caseId || ticketKey || 'SLP-2025-001';
  ...
  const handleNewSession = async () => {
    ...
    try {
      const body = isJira && tKey
        ...
        : { title: oldTitle || "Lloyd's Slip Processing", workflowType: oldWorkflowType || 'slip', ...(oldCaseId && { caseId: oldCaseId }), forceNew: true };
      ...
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center space-y-6 max-w-2xl">
          ...
          <div className="flex justify-center mt-8">
            <div className="max-w-md w-full border rounded-lg p-8 space-y-6 hover:border-primary transition-colors">
              ...
              <div className="text-center space-y-3">
                ...
                <p className="text-muted-foreground">
                  Multi-agent Lloyd's slip workflow with data extraction, validation, risk assessment, pricing, and documentation.
                </p>
              </div>
              <Button 
                ...
                size="lg"
              >
                {createWorkflowMutation.isPending ? "Creating Workflow..." : "Start New Slip Workflow"}
              </Button>
            </div>
          </div>
        </div>
                ...
                {showTimeline ? (
                  <div className="h-full flex flex-col">
                    ...
                    <div className="flex-1 overflow-y-auto">
                      <CaseTimeline
                        caseId={(workflowData as any)?.session?.caseId || (workflowData as any)?.caseId || 'SLP-2025-001'}
                  ...
                  <div className="h-full overflow-y-auto px-4">
                    {(() => {
                      ...
                      const configEndpoint = isJiraForm
                        ...
                        : `/api/slip-forms/data-extraction-config?sessionId=${currentSessionId}`;
