...
import { getSubmissionDataById, getAvailableSubmissionIds } from "@shared/csv-data";
import { getSlipDataById, getAvailableSlipIds } from "@shared/slip-data";
...
export default function Dashboard() {
  ...
  const mockCases: Case[] = useMemo(() => {
    ...
    return dashboardCases.map((dashboardCase: any) => {
      const agentConfigs = dashboardCase.case_type === 'slip' ? [
        ...
        { name: "Data Extraction", type: "policy_extractor", icon: <FileText className="h-4 w-4" />, description: "Extracts property address, coverages, and limits from policy document" },
        { name: "Property Enrichment", type: "property_data", icon: <Building className="h-4 w-4" />, description: "Geocoding, address validation, and CoStar property data fetch" },
        { name: "Risk Assessment", type: "geospatial_risk", icon: <Shield className="h-4 w-4" />, description: "Geospatial, catastrophe, and portfolio concentration risk evaluation" },
        { name: "Quote Generation", type: "quote_generator", icon: <Calculator className="h-4 w-4" />, description: "Generates final premium quote with risk-adjusted breakdown" },
        { name: "Quote Issued", type: "binding", icon: <CheckCircle className="h-4 w-4" />, description: "Quote approved, downloaded/saved, and case closed" }
      ];
      ...
      const agents = agentConfigs.map((config, index) => {
        ...
        const submissionStages = ['Data Extraction', 'Property Enrichment', 'Risk Assessment', 'Quote Generation', 'Quote Issued'];
        const slipStages = ['Data Extraction', 'Data Transformation', 'Quality Assurance', 'Sanctions Check', 'PAS Integration'];
        const stageNames = dashboardCase.case_type === 'slip' ? slipStages : submissionStages;
  ...
  const handleOpenCase = async (caseId: string, caseType: 'submission' | 'slip' = 'submission') => {
    try {
      // For submission (insurance quote) workflows always create fresh so new 7-agent pipeline is used.
      // For slip/jira workflows, resume the existing session.
      if (caseType === 'submission') {
        const createResponse = await fetch('/api/workflows', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            title: `Personal Lines Submission — ${caseId}`,
            caseType,
            caseId,
            forceNew: true
          }),
      ...
      // Slip: always create fresh session (same as submission)
      const createResponse = await fetch('/api/workflows', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const handleCreateNewWorkflow = async (title?: string, caseType: 'submission' | 'slip' = 'submission', caseId?: string) => {
    try {
      const response = await fetch('/api/workflows', {
        ...
        body: JSON.stringify({
          title: title || `New ${caseType === 'submission' ? 'Submission' : 'Slip'} Processing`,
          caseType,
          caseId: caseId || (caseType === 'submission' ? 'UW-2025-001' : 'SLP-2025-001'),
